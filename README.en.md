...
**Smart suggestions.** Six cards on the Profile tab: Largest Files (size badge on every photo in the flow), Burst Photos (timestamp + perceptual-hash double check, sharpest kept automatically), Old Screenshots (90+ days), **Exact Duplicates** (hash + resolution + file size triple match, one copy kept per group), **Duplicate Videos** (duration + resolution + size match, generated thumbnails), and **Low-Quality Photos** (blurry / under- / over-exposed / blank pocket shots).

**Analysis engine.** ONE decode per photo yields a dHash difference hash, Laplacian sharpness and an exposure histogram; metrics live in a global per-asset store persisted **incrementally** — interruptible, shared across albums, never re-analyzed. Chunked processing (50/batch, 10 in low-power mode) with 6-way parallel decodes and a cancellable progress overlay; caches carry an `assetCount` + `modificationTime` fingerprint with a re-analyze prompt on drift; iOS memory warnings pause the loop.

**Sessions that survive restarts.** Entering a cleaning flow captures a "before" snapshot; if the app dies, the next launch offers **Resume or discard** — resume restores the exact shuffled order, group, position and marks. Finishing computes the "after" snapshot for the storage chart and usage stats.

**Info surfaces.** The floating glass info bar shows the capture date and a **reverse-geocoded address** (cached); tapping opens fully localized EXIF details (camera / lens / aperture / shutter / ISO / focal length). All images render through `expo-image` native caching with next-photo prefetch.
...
**Settings.** Global group size, cleaning order (random / by date, default random), similarity toggle, recycle bin toggle (Android only), daily reminder at a random 8AM–8PM time, theme (system / light / dark, instant), and language (**中文 by default**, English available).
...
Album sizes are sampled (first 300 assets, extrapolated); per-album hashing caps at 3,000 photos. Similarity uses a 64-bit dHash with a Hamming threshold of 10 (`SIMILAR_THRESHOLD` in `chunkedAnalyzer.js`); exact duplicates require identical hash, resolution AND file size. "Move to album" adds to the target album on iOS (albums are non-exclusive). Deletion always goes through the OS confirmation dialog — a platform requirement; this app batches it to **once per group**. In the burst cleaner the pre-selected set is deleted and the starred (sharpest) photo is kept.
